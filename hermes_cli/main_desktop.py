...
from hermes_cli.main_web_build import (
    _hash_source_tree, _nixos_build_env, _stamp_is_current, _steamos_glibc_header_env, _write_build_stamp)
...
def _install_desktop_workspace_deps(npm: str, env: dict) -> None:
    ...
    # On SteamOS, layer in -idirafter CFLAGS/CXXFLAGS pointing at a fetched
    # glibc/kernel header set: the immutable root's /usr/include is missing
    # stdint.h et al, which otherwise fails node-pty's native compile with
    # "fatal error: stdint.h: No such file or directory" even though gcc/g++
    # are present.
    build_env = with_hermes_node_path(_nixos_build_env())
    steamos_env = _steamos_glibc_header_env()
    if steamos_env:
        build_env = {**(build_env or {}), **steamos_env}
        # with_hermes_node_path already ran above; re-apply it so the managed-
        # Node PATH isn't lost under steamos_env's full os.environ copy.
